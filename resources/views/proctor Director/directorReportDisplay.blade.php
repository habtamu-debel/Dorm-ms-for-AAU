...
<body>
  <div class="container">
    ...
    <table>
      <thead>
        <tr>
        <th>Number of Students accepted</th>
          <th>Number of Students allocated</th>
      ...
      <tbody>
        @foreach($pmReports as $pmReport)
          <tr>
          <td>{{ $pmReport->numStudentsAccepted }}</td>
            <td>{{ $pmReport->numStudentsAllocated }}</td>
            <td>{{ $pmReport->numStudentClearanced }}</td>
         
            <td>{{ $pmReport->numFreeDorms }}</td>
            <td>
              <a href="{{ route('directorReport.edit', $pmReport->id) }}">Edit</a>
              <form action="{{ route('directorReport.destroy', $pmReport->id) }}" method="POST" style="display: inline;">
                @csrf
                @method('DELETE')
