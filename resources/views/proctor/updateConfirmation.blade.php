...
<div class="form-container">
    <form action="{{ route('resource.update-confirmation', $resource->id) }}" method="POST">
        @csrf
        @method('PUT')

        <label for="status">Status:</label>
        <select name="confirmation" id="confirmation">
            <option value="confirmed" {{ $resource->status === 'confirmed' ? 'selected' : '' }}>Confirmed</option>
            <option value="unconfirmed" {{ $resource->status === 'unconfirmed' ? 'selected' : '' }}>Unconfirmed</option>
