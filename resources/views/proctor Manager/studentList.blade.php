...
<!-- Rest of the page content -->
    ...
    <form action="{{ route('approve-filtered-students') }}" method="POST">
    @csrf
    ...
    <table id="studentsTable" class="table table-striped table-hover">
        ...
        <tbody>
            ...
            <td><input type="checkbox" class="studentCheckbox" name="student_ids[]" value="{{ $student->student_id }}"></td>
                ...
                <td>{{ $student->sex }}</td>
                <td>
    @if ($student->pmstatus == 'sent')
        <span class="badge badge-success" style="width: 70px; height: 30px;">{{ $student->status }}</span>
    @else
        {{ $student->pmstatus }}
    @endif
</td>
...
function filterTable() {
  ...
  const sex = row.cells[5].textContent.toLowerCase(); // Assuming sex is in the 4th column
