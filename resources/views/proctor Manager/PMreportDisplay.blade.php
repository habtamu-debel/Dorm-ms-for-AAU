    ...
    <table>
      ...
      <tbody>
        @foreach($pmReports as $pmReport)
          <tr>
            <td>{{ $pmReport->numStudents }}</td>
            <td>{{ $pmReport->numCases }}</td>
            <td>{{ $pmReport->numMaintenances }}</td>
            <td>{{ $pmReport->numSevereMaintenances }}</td>
            <td>{{ $pmReport->numDormsCleaning }}</td>
            <td>
              <a href="{{ route('pmreports.edit', $pmReport->id) }}">Edit</a>
              <form action="{{ route('pmreports.destroy', $pmReport->id) }}" method="POST" style="display: inline;">
                @csrf
                @method('DELETE')
